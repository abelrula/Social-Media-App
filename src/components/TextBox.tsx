...
const TextBox = ({ placeholder,buttonName }: propsTypes) => {
  ...
  return (
      <span className='flex gap-2'>
        ...
        <textarea className='w-full h-full outline-none text-sm text-start p-2' value={ msg } onChange={ (e) => setMsg(e.target.value) } placeholder={`${placeholder}😀`}  />
            <span className={`ml-2 flex items-center gap-1 z-50`}>
                 <SlPicture className='icon' color='red' />
                  <Emojies setMsg={setMsg} />       
             </span> 
        </span>
         <button className='self-end bg-slate-900 px-2 rounded-lg text-white'  >{buttonName}</button>
    </span>
  )
}
